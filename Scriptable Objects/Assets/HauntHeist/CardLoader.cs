...
namespace HH
{
    public enum CardType { HAUNTED, UNHAUNTED, DEFAULT }
    ...
    public class CardLoader : MonoBehaviour
    {
        ...
        int cardCount = 0;
        Dictionary<string, CardType> card_map = new Dictionary<string, CardType>()
    {
        { "Haunted" , CardType.HAUNTED },
        { "Unhaunted" , CardType.UNHAUNTED },
    };
        ...
        void Populate()
        {
            ...
            for (int y = 1; y < grid.GetLength(1); y++) //2; y++)
            {
                print("GOO GOO");
                if (cardCount == cardSlots.Count)
                {
                    cardCount = 0;
                    Capture();
                ...
                try
                {
                    CardType card_type = card_map.ContainsKey(grid[1, y]) ? card_map[grid[1, y]] : CardType.DEFAULT;
                    cardCount += cardSlots[cardCount].Fill(card_type, grid[0, y], grid[2, y], grid[3, y], grid[5, y], grid[1, y]);
                }
                catch
                {
                    continue;
        ...
        int captureWidth = 240;
        int captureHeight = 336;
        int posX, posY = 0;
    ...
    public class CardLoaderEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            var scripts = targets.OfType<CardLoader>();
